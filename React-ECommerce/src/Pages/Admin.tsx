    ...
    return (
        <div className="admin-container-main">
            <div className="admin-container-admin">
                ...
                {addProductMutation.error && <p className="error-message">Failed to add product: {addProductMutation.error.message}</p>}
                {editProductMutation.error && <p className="error-message">Failed to update product: {editProductMutation.error.message}</p>}
                {deleteProductMutation.error && <p className="error-message">Failed to delete product: {deleteProductMutation.error.message}</p>}
                    ...
                    {products?.map((product) => (
                        <li key={product.id} className="product-card-admin" style={{ listStyle: 'none' }}>
                            ...
                            <div className="product-buttons-admin">
                                <button onClick={() => deleteProductMutation.mutate(product.id)} className="delete-btn-admin">Delete</button>
                                <button onClick={() => { setEditingProduct(product); setProductData(product); }} className="edit-btn-admin">Edit</button>
                            </div>
                        </li>
                    ))}
                </ul>
